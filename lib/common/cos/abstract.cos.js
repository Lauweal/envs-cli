    ...
    uploadFiles(local, remote, files) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const file of files) {
                ...
                percent = parseInt(((index / size) * 100));
                if (status) {
                    logger_1.Logger.info(`[${index}/${size}, ${percent}%] upload remote: ${localFile}`);
                }
                else {
                    logger_1.Logger.wran(`[${index}/${size}, ${percent}%] upload remote: ${localFile}`);
                    paths.push(file);
                }
            }
            return paths;
    ...
    upload(local, remote) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (deleteFiles.size > 0) {
                yield this.cleanRemoteFiles(remote, deleteFiles);
            }
            console.log(local, remote, localFiles);
            const files = yield this.uploadFiles(local, remote, localFiles);
            if (files.length)
